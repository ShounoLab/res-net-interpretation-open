...
for i in `seq 0 $((${#SEEDS[@]} - 1))`; do
    ...
    # for LAYER_NUM in `seq 3 4`; do
        ...
        # for LAYER_SIZE in `seq 4 $((LAYER_SIZE_ARRAY[LAYER_NUM - 1]))`; do
            for j in `seq 0 $((${#ARCH[@]} - 1))`; do
                ...
                echo ${ARCH[$j]}
                echo $LAYER
                OUT="/data2/genta/resnet/analysis/optimalinput-clip"$optim"-lr"$LR"-wd"$WD
                python analyize_optimalinput.py --out $OUT --arch ${ARCH[$j]} --layer-name $LAYER  --device "cuda:1" --lr $LR --mode neuron --max-iter $MAX_ITER  --seed $seed  --batch-size 256 --wd $WD --optim $optim --zero-start --off-sorted-channel --clip-inputspace
            done
        done
    done 
done
